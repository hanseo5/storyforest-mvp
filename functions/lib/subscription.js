...
exports.portOneWebhook = exports.cancelSubscription = exports.subscribeToPlan = void 0;
const functions = __importStar(require("firebase-functions"));
const admin = __importStar(require("firebase-admin"));
const portone_1 = require("./portone");
const db = admin.firestore();
const portOne = new portone_1.PortOneClient();
const PLANS = {
    'storyteller': {
...
exports.portOneWebhook = functions.https.onRequest(async (req, res) => {
    ...
    try {
        if (status === 'paid') {
            ...
            const plan = PLANS[planId] || PLANS['storyteller'];
            // Extend Subscription
            const nextBillingDate = new Date();
            nextBillingDate.setMonth(nextBillingDate.getMonth() + 1);
            const scheduleAt = Math.floor(nextBillingDate.getTime() / 1000);
